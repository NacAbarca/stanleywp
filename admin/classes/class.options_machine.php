...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'slider':
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">Agregar nueva diapositiva</a></div>';
				...
				case 'backup':
					...
					if(!isset($backup['backup_log'])) {
						$log = 'Aún no hay copias de seguridad';
					} else {
					...
					$output .= '<p><strong>'. __('Última copia de seguridad : ').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
					$output .= '<a href="#" id="of_backup_button" class="button" title="Opciones de respaldo">Opciones de respaldo</a>';
					$output .= '<a href="#" id="of_restore_button" class="button" title="Opciones de restauración">Opciones de restauración</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$output .= '<a href="#" id="of_import_button" class="button" title="Opciones de restauración">Opciones de importación</a>';
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">Subir</span>';

			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">Eliminar</span>';
		}
		else
		{
			$output .= '<p class="upload-notice"><i>Actualice su versión de WordPress para obtener soporte total de los medios.</i></p>';
		}
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		$slider .= '<a class="slide_edit_button" href="#">Editar</a></div>';

		$slider .= '<div class="slide_body">';

		$slider .= '<label>Título</label>';
		...
		$slider .= '<label>URL de la imagen</label>';
		$slider .= '<input class="upload slide of-input" name="'. $id .'['.$order.'][url]" id="'. $id .'_'.$order .'_slide_url" value="'. $val['url'] .'" />';

		$slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.$id.'_'.$order .'">Subir</span>';

		if(!empty($val['url'])) {$hide = '';} else { $hide = 'hide';}
		$slider .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'_'.$order .'" title="' . $id . '_'.$order .'">Eliminar</span>';
		$slider .='</div>' . "\n";
		...
		$slider .= '</div>';
		$slider .= '<label>URL del enlace (optional)</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link'] .'" />';

		$slider .= '<label>Descripción (optional)</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';

		$slider .= '<a class="slide_delete_button" href="#">Borrar</a>';
